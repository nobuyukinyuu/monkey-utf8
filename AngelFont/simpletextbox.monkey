...
'					Local chars:Char[] = font.GetChars()
	...
	'UTF8String override, takes char array  -nobu
	Function Draw:Void(text:Int[], x:Int, y:Int, width:Int, alignment:Int = AngelFont.ALIGN_LEFT)
		
		Local thisLine:= New IntStack()
		Local charOffset:Int = 0
		
		Local wordLen:Int = 0
		Local word:= New IntStack()
		...
		For Local i := 0 Until text.Length
			...
			Select asc
				Case 32	' space
					wordLen = font.TextWidth(word.ToArray())
					If charOffset + wordLen > width
						DrawTextLine(thisLine.ToArray(), x, y + yOffset)
						thisLine.Clear()
						charOffset = 0
					Endif
'					Local chars:Char[] = font.GetChars()
					charOffset += wordLen+font.GetChars()[32].xAdvance
					thisLine.Push(word.ToArray()); thisLine.Push(32) 'thisline += word + " "
				...
				Case 10	' newline or "~n"
					...
					If charOffset + wordLen > width
						...
						thisLine.Clear()
					Endif
					thisLine.Push(word.ToArray()) 'thisLine += word
		...
		If Not word.IsEmpty()  'If word <> ""
			...
			thisLine.Push(word.ToArray()) 'thisLine += word
		EndIf
		If Not thisLine.IsEmpty()  'If thisLine <> ""
			DrawTextLine(thisLine.ToArray(), x, y + yOffset)
		Endif
	End
...
'					Local chars:Char[] = font.GetChars()
	...
	End
	Function DrawTextLine:Void(txt:Int[], x:Int, y:Int)
		font.DrawText(txt, x,y, align)
		yOffset += lineGap+font.height
	End
