...
'	Field blockSize:Int
	...
	Field extChars:= New IntMap<Char>
...
'			Print " -> "+node.GetName()+"(id="+node.GetAttribute("id")+" x="+node.GetAttribute("x")+" y="+node.GetAttribute("y")+" width="+node.GetAttribute("width")+" height="+node.GetAttribute("height")+" )"
			...
			Local c:Char = New Char(Int(node.GetAttribute("x")), Int(node.GetAttribute("y")),
				 Int(node.GetAttribute("width")), Int(node.GetAttribute("height")),
				  Int(node.GetAttribute("xoffset")), Int(node.GetAttribute("yoffset")),
				   Int(node.GetAttribute("xadvance")), page)
			If id < 256 And id >= 0 'Hack to support unicode  -nobu
				chars[id] = c
			Else
				'Print "charID " + id
				extChars.Add(id, c)
			...
			If c.height > Self.height Self.height = c.height
			If c.yOffset < Self.heightOffset Self.heightOffset = c.yOffset
		Next
...
'		Local prevChar:String = ""
		...
		For Local i:= 0 Until txt.Length
			...
			'Unicode hack  -nobu
			If asc < 256 Then ac = chars[asc] Else ac = extChars.Get(asc)
...
'							Print prevChar+","+thisChar
	...
	'UTF8 Override method!  -nobu
	Method DrawText:Void(txt:Int[], x:Int, y:Int)
		Local prevChar:Int = 0
		xOffset = 0
...
'							Print prevChar+","+thisChar
	...
	Method DrawText:Void(txt:String, x:Int, y:Int, align:Int)
		...
		End Select
	End Method
	Method DrawText:Void(txt:Int[], x:Int, y:Int, align:Int)
		xOffset = 0
		Select align
			Case ALIGN_CENTER
				DrawText(txt,x-(TextWidth(txt)/2),y)
...
'			Local thisChar:String = String.FromChar(asc)
		...
		Return width
	End Method
	Method TextWidth:Int(txt:Int[])   'Unicode codepoint override  -nobu
'		Local prevChar:String = ""
		Local prevChar:Int = 0
		Local width:Int = 0
		For Local i:= 0 Until txt.Length
			Local codePoint:Int = txt[i]
			Local ac:Char
			
			If codePoint < 256 Then ac = chars[codePoint] Else ac = extChars.Get(codePoint)

'			Local thisChar:String = String.FromChar(codePoint)
			Local thisChar:Int = codePoint
			If ac  <> Null
				If useKerning
					Local firstKp:= kernPairs.Get(prevChar)
					If firstKp <> Null
